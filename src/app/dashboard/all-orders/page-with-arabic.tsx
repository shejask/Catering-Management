...
export default function AllOrdersPage() {
  ...
  const onExportSubmit = (data: ExportFormData) => {
    ...
    if (filteredData.length === 0) {
      toast.error(t('message.noOrdersInRange'));
      return;
    }

    // Ask user for confirmation
    if (!confirm(t('export.confirmExport').replace('{count}', filteredData.length.toString()).replace('{startDate}', data.startDate).replace('{endDate}', data.endDate))) {
      return;
    }

    const filename = `almakarem_orders_${data.startDate}_to_${data.endDate}.csv`;
    exportToCSV(filteredData, filename);
    toast.success(t('message.exportSuccess').replace('{count}', filteredData.length.toString()));
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      toast.error(t('message.fetchError'));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  const updateOrderStatus = async (orderId: string, status: 'paid' | 'unpaid') => {
    try {
      ...
      const statusText = status === 'paid' ? t('status.paid') : t('status.unpaid');
      toast.success(t('message.statusUpdated').replace('{status}', statusText));
    } catch (error) {
      // eslint-disable-next-line no-console
  ...
  const shareOrderToCook = async (orderId: string) => {
    try {
      ...
      toast.success(t('message.sharedWithCook'));
    } catch (error) {
      // eslint-disable-next-line no-console
  ...
  const onEditSubmit = async (data: EditFormData) => {
    ...
    try {
      ...
      toast.success(t('message.orderUpdated'));
            ...
            <div className='flex flex-wrap gap-4'>
              ...
              <div className='flex items-end'>
                <div className='text-sm text-muted-foreground'>
                  {t('filter.showingResults').replace('{count}', filteredOrders.length.toString()).replace('{total}', orders.length.toString())}
                </div>
              </div>
            </div>
          </CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{t('table.advancePaid')}</TableHead>
                    <TableHead>{t('table.total')}</TableHead>
                    <TableHead>{t('table.status')}</TableHead>
                    <TableHead>{t('table.cookStatus')}</TableHead>
                  ...
                  {filteredOrders.map((order) => {
                    ...
                    return (
                      <TableRow key={order.orderId}>
                        ...
                        <TableCell>{formatCurrency(order.advancePayment)}</TableCell>
                        <TableCell>{formatCurrency(finalTotal)}</TableCell>
                        <TableCell>
                          <Badge 
                        ...
                        <TableCell>
                          <Badge 
                            ...
                            className={`cursor-pointer ${getCookStatusColor(order.cookStatus || 'pending')}`}
                          >
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      {t('delete.description').replace('{name}', order.name)}
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
        ...
        <Dialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
          <DialogContent className='max-w-2xl'>
            <DialogHeader>
              ...
              <DialogDescription>
                {t('dialog.orderDetailsFor')}{selectedOrder?.orderId}
              </DialogDescription>
            </DialogHeader>
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className='max-w-4xl max-h-[90vh] overflow-y-auto'>
            <DialogHeader>
              ...
              <DialogDescription>
                {t('dialog.updateOrderFor')}{editingOrder?.orderId}
              </DialogDescription>
            </DialogHeader>
